    ...
    <>
      ...
      <section data-visual-state="0" className="relative isolate -mt-16 flex min-h-[100svh] items-center overflow-hidden bg-transparent text-ink lg:-mt-[72px]">
        <div className="container-page relative z-10 w-full pb-14 pt-28 sm:pt-32 lg:pb-16 lg:pt-32">
          <span className={cn('eyebrow reveal-up', shown && 'is-in')} style={rd(60)}>{home.hero.eyebrow}</span>
          <h1 className="text-display mt-7 text-ink text-[clamp(2.35rem,5.6vw,4.5rem)]">
            {titleLines.map((line, i) => (
              <span key={i} className={cn('reveal-line block', shown && 'is-in')} style={rd(180 + i * 90)}><span>{line}</span></span>
            ))}
            <span className={cn('reveal-line block', shown && 'is-in')} style={rd(180 + titleLines.length * 90)}><span className="text-brand">{home.hero.titleAccent}</span></span>
          </h1>

          <div className="mt-12 grid gap-x-8 gap-y-12 lg:mt-16 lg:grid-cols-12">
            {home.hero.subtitle ? (
              <p className={cn('reveal-up self-end text-[0.98rem] leading-relaxed text-ink-muted lg:col-span-4', shown && 'is-in')} style={rd(560)}>{home.hero.subtitle}</p>
            ) : null}
            <div className={cn('reveal-fade lg:col-span-5 lg:col-start-8', shown && 'is-in')} style={rd(680)}><BusinessAutomationDemo /></div>
          </div>
          ...
          <div className="mt-12 flex flex-col gap-9 border-t border-line pt-8 lg:mt-12 lg:flex-row lg:items-center lg:justify-between">
            <div className={cn('reveal-up flex flex-col gap-3 sm:flex-row', shown && 'is-in')} style={rd(820)}>
              <Link href="/consult" className="btn-primary">{home.hero.primaryCta}<Arrow className="h-4 w-4" /></Link>
              <Link href="/services" className="btn-outline">{home.hero.secondaryCta}</Link>
            </div>
            <dl className={cn('reveal-up flex flex-wrap gap-x-9 gap-y-4', shown && 'is-in')} style={rd(900)}>
              {stats.map((s) => (
                <div key={s.label} className="flex items-baseline gap-2.5"><dt className="text-display text-2xl text-brand">{s.value}</dt><dd className="max-w-[9rem] text-xs font-medium leading-tight text-ink-faint">{s.label}</dd></div>
              ))}
            </dl>
          </div>
        </div>
      ...
      <div className="relative bg-transparent text-ink">
        <section data-visual-state="1" className="relative overflow-hidden">
          <div className="container-page relative z-10 py-12 sm:py-14 lg:py-16"><IntegrationRail /></div>
        </section>

        <div data-visual-state="2" className="relative"><Capabilities /></div>

        <section data-visual-state="3" className="relative overflow-hidden">
          <div className="container-page relative z-10 border-t border-line py-12 sm:py-14 lg:py-16"><ProblemShift /></div>
        </section>

        <div data-visual-state="4" className="relative"><BeforeAfterAutomation /></div>

        <section data-visual-state="5" className="relative overflow-hidden">
          <div className="container-page relative z-10 border-t border-line py-12 sm:py-14 lg:py-16"><HomeFaq /></div>
        </section>

        <section data-visual-state="6" className="relative overflow-hidden">
          <div className="container-page relative z-10 border-t border-line py-12 sm:py-14 lg:py-16"><HomeCta /></div>
        </section>
      </div>
    </>
  );
        ...
        {t.items.map((item, index) => {
          ...
          return (
            <div key={item.q} className={cn('border-b py-1 transition-colors duration-500', open ? 'border-[hsl(var(--brand)/0.38)]' : 'border-line')}>
              <dt><button type="button" onClick={() => setOpenIndex(open ? null : index)} aria-expanded={open} aria-controls={answerId} className="group flex min-h-16 w-full items-center justify-between gap-6 py-4 text-start focus-visible:outline-none">
                <span className={cn('text-display text-[clamp(1rem,2vw,1.2rem)] transition-[color,transform] duration-500', open ? 'translate-x-1 text-brand rtl:-translate-x-1' : 'text-ink group-hover:text-brand')}>{item.q}</span>
                <span aria-hidden="true" className={cn('h-5 w-5 shrink-0 transition-[color,transform] duration-500', open ? 'rotate-45 text-brand' : 'text-ink-faint group-hover:text-brand')}><svg viewBox="0 0 20 20" fill="none" className="h-full w-full"><path d="M10 3v14M3 10h14" stroke="currentColor" strokeWidth="1.35" strokeLinecap="round" /></svg></span>
              </button></dt>
              <dd id={answerId} aria-hidden={!open} className={cn('grid max-w-2xl transition-[grid-template-rows,opacity] duration-500 ease-out', open ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0')}>
                <div className="overflow-hidden"><p className="pb-5 pe-10 text-sm leading-relaxed text-ink-muted transition-transform duration-500 ease-out" style={{ transform: open ? 'translateY(0)' : 'translateY(-0.45rem)' }}>{item.a}</p></div>
              </dd>
            </div>
          );
        })}
  ...
  return (
    <div className="max-w-3xl">
      ...
      <div className="mt-9 flex flex-col gap-3 sm:flex-row">
        <Link href="/consult" className="btn-primary">{t.primaryCta}<Arrow className="h-4 w-4" /></Link>
